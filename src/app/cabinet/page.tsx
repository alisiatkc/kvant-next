...
import {
  ...
  Cpu, ArrowLeft, CheckCircle, XCircle, X,
  LayoutDashboard, ClipboardList, FolderOpen, MessageSquare, BookOpen,
  Send, Calendar, Lightbulb, Hammer, School, BarChart3,
} from 'lucide-react'
...
type ChatMessage = {
  id: string; author: string; text: string; time: string; isSystem?: boolean
}
...
const DEFAULT_TASKS: Task[] = [
  { id: '1', title: 'Выбрать тему КОП', desc: 'Обсудить с куратором и командой', status: 'done',       priority: 'high',   dueDate: '' },
  { id: '2', title: 'Подготовить макет', desc: 'Создать файл DXF для лазерной резки',  status: 'inprogress', priority: 'high',   dueDate: '' },
  { id: '3', title: 'Написать инструкцию', desc: 'Методика использования для педагога', status: 'planned',    priority: 'medium', dueDate: '' },
  { id: '4', title: 'Сфотографировать КОП', desc: 'Фото для публикации в каталоге',     status: 'planned',    priority: 'low',    dueDate: '' },
]

const DEFAULT_FILES: FileItem[] = [
  { name: 'Макет_v1.dxf',        icon: 'File',     size: '0.4 МБ', addedAt: '2024-01-10' },
  { name: 'Инструкция.pdf',      icon: 'FileText', size: '1.2 МБ', addedAt: '2024-01-12' },
  { name: 'Схема_соединений.ino', icon: 'File',     size: '0.1 МБ', addedAt: '2024-01-14' },
]
...
export default function CabinetPage() {
  ...
  const handleLogin = () => {
    setLoginError('')
    if (userType === 'student') {
      if (!captainName.trim() || !teamName.trim()) { setLoginError('Заполните все поля'); return }
  ...
  const savePassport = () => {
    try {
      localStorage.setItem('cabinet_projectName',    projectName)
      localStorage.setItem('cabinet_projectBlock',   projectBlock)
      localStorage.setItem('cabinet_projectDesc',    projectDesc)
      localStorage.setItem('cabinet_productionFile', productionFile)
    } catch {}
  ...
  const deleteTask = (id: string) => {
    if (confirm('Удалить задачу?')) setTasks((p) => p.filter((t) => t.id !== id))
  }
  ...
  const addFile = () => {
    ...
    if (!name?.trim()) return
    const f: FileItem = { name: name.trim(), icon: name.endsWith('.pdf') ? 'FileText' : 'File', size: '—', addedAt: new Date().toISOString().slice(0, 10) }
    setFiles((p) => [...p, f])
  }

  const deleteFile = (i: number) => {
    if (confirm('Удалить файл?')) setFiles((p) => p.filter((_, idx) => idx !== i))
  }
  ...
  const sendMessage = () => {
    ...
    setChatInput('')
    try { localStorage.setItem('cabinet_chat', JSON.stringify(next.filter((m) => !m.isSystem))) } catch {}
  }
  ...
  const stages = [
    ...
    { Icon: School,     label: 'Апробация',    hint: 'КОП опубликован' },
    { Icon: BarChart3,  label: 'Рефлексия',    hint: 'Апробация пройдена' },
  ]

  const visibleTabs = NAV.filter((t) => !t.onlyPublished || published)
      ...
      <>
        ...
        <main>
          <div className="bg-white rounded-[3rem] px-[50px] py-[60px] max-w-[550px] mx-auto my-16 shadow-[0_20px_40px_-15px_rgba(0,0,0,0.1)]">
            <Link href="/" className="flex items-center gap-2 text-kv-blue font-medium no-underline mb-8">
              <ArrowLeft className="w-4 h-4" /> На главную
            </Link>
            <h2 className="text-[2.4rem] font-medium mb-2">Личный кабинет</h2>
            <p className="text-kv-text mb-10">Выберите роль для входа</p>

            <div className="flex gap-5 mb-8 justify-center">
              {(['student', 'curator'] as const).map((t) => (
                <label key={t} className="flex items-center gap-2 cursor-pointer">
                  <input type="radio" name="userType" checked={userType === t} onChange={() => setUserType(t)} />
            ...
            {userType === 'student' ? (
              <>
                <div className="flex gap-4 mb-10">
                  {(['А1', 'А2'] as const).map((t) => (
                    <button key={t}
                      className={`flex-1 py-4 rounded-full border font-medium text-lg cursor-pointer transition-colors ${track === t ? 'bg-kv-blue text-white border-kv-blue' : 'bg-white border-[#e2e8f0] hover:bg-kv-light'}`}
                      onClick={() => setTrack(t)}
                    >
              ...
              <>
                <div className="mb-6">
                  ...
                  <input className="input-kv" placeholder="curator" value={curatorLogin} onChange={(e) => setCuratorLogin(e.target.value)} />
                </div>
                <div className="mb-6">
                  <label className="block mb-2 font-medium text-[#3f4a6b]">Пароль</label>
                  <input className="input-kv" type="password" placeholder="••••••••" value={curatorPassword} onChange={(e) => setCuratorPassword(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleLogin()} />
                </div>
              </>
            )}

            {loginError && <p className="text-[#c62828] text-center mb-4 text-sm">{loginError}</p>}
            <button className="w-full py-4 bg-kv-dark text-white rounded-full text-lg font-medium cursor-pointer hover:bg-kv-blue transition-colors border-none mt-4" onClick={handleLogin}>
              Войти
            </button>
          </div>
        </main>
      ...
      <main>
        <div className="container-kv py-8">
          ...
          <div className="bg-white rounded-[2rem] px-8 py-5 mb-6 flex items-center justify-between flex-wrap gap-4">
            <div>
              <span className="text-kv-muted text-xs uppercase tracking-wide block mb-0.5">Личный кабинет</span>
              <span className="text-xl font-medium">{captainName} <span className="text-kv-muted font-normal">· {teamName}</span></span>
            </div>
            <div className="flex items-center gap-2.5 flex-wrap">
              <span className="tag-kv">{track}</span>
              <span className={`status-badge ${published ? `${sc.bg} ${sc.color}` : 'bg-kv-light text-kv-muted'} flex items-center gap-1.5`}>
                {published ? <><sc.Icon className="w-4 h-4" /> {sc.label}</> : 'Черновик'}
              </span>
            </div>
          </div>
          ...
          <div className="flex gap-6 items-start max-[920px]:flex-col">
            ...
            <nav className="w-[200px] flex-shrink-0 space-y-1 max-[920px]:w-full max-[920px]:flex max-[920px]:flex-wrap max-[920px]:gap-2 max-[920px]:space-y-0">
              {visibleTabs.map(({ id, label, Icon }) => (
                <button key={id}
                  className={`flex items-center gap-3 w-full px-4 py-3 rounded-2xl text-left text-[0.9rem] cursor-pointer border-none transition-all max-[920px]:w-auto max-[920px]:flex-1
                    ${activeTab === id ? 'bg-kv-blue text-white font-medium' : 'bg-white text-kv-dark hover:bg-kv-light'}`}
            ...
            <div className="flex-1 min-w-0 space-y-5">
              ...
              {activeTab === 'overview' && (
                <>
                  ...
                  <div className="bg-white rounded-[2.5rem] p-8">
                    <h3 className="text-[1.4rem] font-medium mb-7">Прогресс проекта</h3>
                  ...
                  <div className="grid grid-cols-2 min-[560px]:grid-cols-4 gap-4">
                    {[
                      { label: 'Задач выполнено', value: `${doneTasks}/${totalTasks}`, color: 'text-kv-blue' },
                      ...
                      <div key={s.label} className="bg-white rounded-[1.75rem] p-6 text-center border border-kv-border">
                        <div className={`text-[1.9rem] font-semibold ${s.color} mb-1`}>{s.value}</div>
                  ...
                  {published && (
                    <div className={`rounded-[1.75rem] p-6 flex items-start gap-4 ${sc.bg}`}>
                      ...
                      <div>
                        ...
                        {projectStatus === 'review'   && <p className="text-sm text-kv-muted mt-1">Куратор рассматривает проект — обычно 1–2 рабочих дня.</p>}
                  ...
                  {/* Quick actions */}
                  <div className="bg-white rounded-[2.5rem] p-8">
                    <h4 className="font-medium mb-5 text-[1.1rem]">Быстрые действия</h4>
                    <div className="flex flex-wrap gap-3">
                      ...
                      {!published && (
                        <button className="bg-[#e8f5e9] text-[#2e7d32] border border-[#86efac] rounded-full px-5 py-3 text-sm font-medium cursor-pointer hover:bg-[#c8e6c9] transition-colors flex items-center gap-2" onClick={handlePublish}>
                          <CheckCircle2 className="w-4 h-4" />Опубликовать КОП
                        </button>
                      )}
                      {published && projectStatus === 'approved' && (
                        <button className="btn-ai text-sm" onClick={() => setActiveTab('approbation')}><School className="w-4 h-4" />Добавить апробацию</button>
                      )}
                    </div>
                  </div>
                </>
              ...
              {activeTab === 'passport' && (
                <div className="bg-white rounded-[2.5rem] p-8">
                  <h3 className="text-[1.4rem] font-medium mb-1">Паспорт проекта</h3>
                    ...
                    <div className="flex flex-wrap gap-3 mb-4">
                      {authors.map((a, i) => (
                        <span key={i} className="bg-kv-light px-4 py-2 rounded-full flex items-center gap-2 text-sm">
                          <User className="w-4 h-4" /> {a}{i === 0 && <span className="text-kv-muted text-xs">капитан</span>}
                        </span>
                      ))}
                    </div>
                  ...
                  {passportSaved && <div className="mt-4 bg-[#e8f5e9] px-5 py-3 rounded-2xl text-[#2e7d32] text-sm">✅ Паспорт сохранён</div>}
              ...
              {activeTab === 'tasks' && (
                <div className="bg-white rounded-[2.5rem] p-8">
                  <div className="flex justify-between items-center mb-7">
                    <div>
                      <h3 className="text-[1.4rem] font-medium">Трекер задач</h3>
                      <p className="text-kv-muted text-sm mt-0.5">{doneTasks} из {totalTasks} выполнено</p>
                    </div>
                  ...
                  <div className="grid grid-cols-3 gap-4 max-[700px]:grid-cols-1">
                    {columns.map(({ status, label, Icon }) => (
                      <div key={status} className="bg-[#f9fbfe] rounded-[1.75rem] p-5 min-h-[300px]"
                          ...
                          return (
                            <div key={task.id}
                              ...
                              <div className="flex justify-end gap-1 mt-1">
                                <button className="bg-transparent border-none cursor-pointer text-[#8b9bb5] hover:text-kv-blue p-1 transition-colors" onClick={() => openTask(task)}>
                                  <Edit2 className="w-3.5 h-3.5" />
                                </button>
                                <button className="bg-transparent border-none cursor-pointer text-[#8b9bb5] hover:text-red-400 p-1 transition-colors" onClick={() => deleteTask(task.id)}>
                        ...
                        {tasks.filter((t) => t.status === status).length === 0 && (
                          <div className="flex items-center justify-center h-16 text-kv-muted text-xs">Перетащите задачу сюда</div>
                        )}
                      </div>
                    ))}
                  </div>
              ...
              {activeTab === 'files' && (
                <div className="bg-white rounded-[2.5rem] p-8">
                  <div className="flex justify-between items-center mb-7">
                    <div>
                      <h3 className="text-[1.4rem] font-medium">Рабочие файлы</h3>
                  ...
                  {files.length === 0 ? (
                    <div className="text-center py-16 text-kv-muted"><FolderOpen className="w-10 h-10 mx-auto mb-3 opacity-30" /><p className="text-sm">Нет загруженных файлов</p></div>
                  ) : (
                    <div className="grid grid-cols-[repeat(auto-fill,minmax(150px,1fr))] gap-4">
                      {files.map((f, i) => (
                        <div key={i} className="bg-[#f9fbfe] rounded-[1.75rem] p-5 border border-kv-border relative group hover:bg-kv-light hover:-translate-y-0.5 transition-all">
                          <button className="absolute top-2.5 right-2.5 opacity-0 group-hover:opacity-100 transition-opacity bg-white border-none rounded-full p-1 cursor-pointer text-[#8b9bb5] hover:text-red-400" onClick={() => deleteFile(i)}>
                            <Trash2 className="w-3.5 h-3.5" />
                          </button>
                          <div className="cursor-pointer text-center" onClick={() => alert(`Скачивание ${f.name} (демо)`)}>
                            ...
                            {f.addedAt && <span className="text-[#8b9bb5] text-[0.65rem] block mt-0.5">{f.addedAt}</span>}
                          </div>
                        </div>
                      ))}
                    </div>
              ...
              {activeTab === 'chat' && (
                <div className="bg-white rounded-[2.5rem] p-8 flex flex-col" style={{ height: '580px' }}>
                  <h3 className="text-[1.4rem] font-medium mb-0.5">Командный чат</h3>
                  ...
                  <div className="flex gap-3">
                    <input className="input-kv flex-1" placeholder="Написать сообщение..." value={chatInput}
                      onChange={(e) => setChatInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && !e.shiftKey && sendMessage()} />
              ...
              {activeTab === 'notes' && (
                <div className="bg-white rounded-[2.5rem] p-8">
                  <h3 className="text-[1.4rem] font-medium mb-1">Заметки проекта</h3>
                  ...
                  {notesSaved && <div className="mt-3 bg-[#e8f5e9] px-5 py-3 rounded-2xl text-[#2e7d32] text-sm">✅ Заметки сохранены</div>}
                  <button className="bg-kv-blue text-white border-none rounded-full px-9 py-3.5 text-base font-medium cursor-pointer hover:bg-kv-dark transition-colors mt-5"
                    onClick={() => { try { localStorage.setItem('cabinet_notes', notes) } catch {} setNotesSaved(true); setTimeout(() => setNotesSaved(false), 2500) }}>
                    Сохранить
                  </button>
                </div>
              )}
                ...
                <div className="space-y-5">
                  ...
                  <div className="bg-white rounded-[2.5rem] p-8">
                    <div className="flex justify-between items-start flex-wrap gap-4 mb-5">
                      <div>
                        <h3 className="text-[1.4rem] font-medium mb-1">{projectName || 'Мой КОП'}</h3>
                    ...
                    {productionFile && <p className="mt-3 text-sm flex items-center gap-2 text-kv-blue"><Paperclip className="w-4 h-4" />{productionFile}</p>}
                  </div>

                  {/* Add approbation */}
                  <div className="bg-white rounded-[2.5rem] p-8">
                    <h3 className="text-[1.4rem] font-medium mb-1">Добавить результат апробации</h3>
                    ...
                    <div className="grid grid-cols-2 gap-5 max-[700px]:grid-cols-1">
                      ...
                      <div>
                        ...
                        <input className="input-kv" type="number" min="1" max="5" placeholder="5" value={approbationForm.engagement} onChange={(e) => setApprobationForm((f) => ({ ...f, engagement: e.target.value }))} />
                      </div>
                      {[
                        { key: 'whatWorked',      label: 'Что прошло хорошо?' },
                        { key: 'whatNeedsWork',   label: 'Что требует доработки?' },
                        { key: 'recommendations', label: 'Рекомендации автору' },
                      ].map(({ key, label }) => (
                    ...
                    {approbationSaved && <div className="mt-4 bg-[#e8f5e9] px-5 py-3 rounded-2xl text-[#2e7d32] text-sm">✅ Результаты апробации сохранены</div>}
                  ...
                  {approbationHistory.length > 0 && (
                    <div className="bg-white rounded-[2.5rem] p-8">
                      <h3 className="text-[1.4rem] font-medium mb-5">История апробаций ({approbationHistory.length})</h3>
                        ...
                        {approbationHistory.map((rec) => (
                          <details key={rec.id} className="bg-[#f9fbfe] rounded-[1.75rem] px-6 py-4 border border-kv-border">
                            ...
                            <div className="mt-4 pt-4 border-t border-[#e2e8f0] space-y-2 text-sm text-kv-text">
                              <p><strong className="text-kv-dark">Что удалось:</strong> {rec.whatWorked}</p>
                              <p><strong className="text-kv-dark">Требует доработки:</strong> {rec.whatNeedsWork}</p>
                              <p><strong className="text-kv-dark">Рекомендации:</strong> {rec.recommendations}</p>
                            </div>
                          </details>
                        ))}
                      </div>
              ...
              {/* Publish CTA — shown on all tabs except overview when not yet published */}
              {!published && activeTab !== 'overview' && activeTab !== 'approbation' && (
                <div className="bg-kv-light rounded-[2rem] px-7 py-6 flex items-center justify-between flex-wrap gap-4">
                  <div>
                    <h4 className="font-medium mb-0.5">Готово? Опубликуйте КОП в каталог</h4>
                    <p className="text-kv-muted text-sm">Проект отправится куратору на проверку</p>
                  </div>
                  <button className="bg-kv-blue text-white border-none rounded-full px-8 py-3 text-sm cursor-pointer hover:bg-kv-dark transition-colors" onClick={handlePublish}>
                    Опубликовать
                  </button>
                </div>
              )}
              ...
              <div className="bg-kv-light rounded-[2rem] p-7 text-center">
                <h4 className="font-medium mb-2">Каталог КОП</h4>
                <p className="text-kv-muted text-sm mb-4">Посмотрите проекты других студентов</p>
      ...
      {showAuthorModal && (
        <div className="modal-overlay" onClick={() => setShowAuthorModal(false)}>
          <div className="bg-white max-w-[420px] w-[90%] rounded-[2.5rem] p-10 relative" onClick={(e) => e.stopPropagation()}>
            ...
            <input className="input-kv mb-5" placeholder="Имя и фамилия" value={newAuthorInput} onChange={(e) => setNewAuthorInput(e.target.value)} autoFocus
      ...
      {showTaskModal && (
        <div className="modal-overlay" onClick={() => setShowTaskModal(false)}>
          <div className="bg-white max-w-[480px] w-[90%] rounded-[2.5rem] p-10 relative" onClick={(e) => e.stopPropagation()}>
            ...
            <h3 className="text-[1.8rem] font-medium mb-6">{editingTask ? 'Редактировать' : 'Новая задача'}</h3>
            ...
            <div className="mb-4">
              <label className="block mb-2.5 text-sm font-medium text-[#3f4a6b]">Приоритет</label>
            ...
            <div className="mb-6">
              <label className="block mb-2.5 text-sm font-medium text-[#3f4a6b]">Срок выполнения</label>
      ...
      {showNotification && (
        <div className="modal-overlay" onClick={() => setShowNotification(false)}>
          <div className="bg-white max-w-[420px] w-[90%] rounded-[2.5rem] p-10 text-center" onClick={(e) => e.stopPropagation()}>
            ...
            <h3 className="text-[1.8rem] font-medium mb-3">КОП отправлен!</h3>
            <p className="text-kv-muted mb-6">Куратор получил ваш проект на проверку. Следите за статусом во вкладке «Обзор».</p>
            <button className="w-full py-3.5 bg-kv-blue text-white rounded-full border-none cursor-pointer font-medium hover:bg-kv-dark transition-colors"
