          ...
          <div className="flex gap-6 items-start max-[920px]:flex-col">
            ...
            <div className="flex-1 min-w-0 space-y-5">
              ...
              {activeTab === 'workshops' && (
                <div className="space-y-5">
                  <div className="bg-white rounded-[2.5rem] p-8">
                    <h3 className="text-[1.3rem] font-semibold mb-1">Мастер-классы технопарка</h3>
                    <p className="text-kv-muted text-sm mb-7">Нажмите на карточку, чтобы посмотреть видео и описание</p>
                    <div className="grid grid-cols-1 min-[480px]:grid-cols-2 min-[900px]:grid-cols-3 gap-5">
                      {workshops.map((w) => {
                        const WIcon = WORKSHOP_ICON_MAP[w.icon] ?? Box
                        return (
                          <div key={w.title}
                            className="bg-kv-light rounded-[1.75rem] p-6 border border-kv-border cursor-pointer transition-all hover:-translate-y-1 hover:shadow-card-hover flex flex-col"
                            onClick={() => setSelectedWorkshop(w)}>
                            <WIcon className="w-9 h-9 text-kv-blue mb-4 stroke-[1.5]" />
                            <h4 className="text-[1.05rem] font-medium mb-2 leading-snug">{w.title}</h4>
                            <p className="text-[#4b5672] font-light text-sm leading-relaxed flex-grow">{w.desc}</p>
                          </div>
                        )
                      })}
                    </div>
                  ...
                  {selectedWorkshop && (
                    <div className="modal-overlay" onClick={() => setSelectedWorkshop(null)}>
                      <div className="modal-box" onClick={(e) => e.stopPropagation()}>
                        <button className="modal-close-btn" onClick={() => setSelectedWorkshop(null)}>
                          <X size={24} />
                        </button>
                        <h3 className="text-2xl font-medium mb-6">{selectedWorkshop.title}</h3>
                        <div className="w-full h-[200px] bg-kv-light rounded-[1.5rem] mb-6 overflow-hidden">
                          <iframe width="100%" height="100%"
                            ...
                            allowFullScreen className="rounded-[1.5rem]" />
                        </div>
                        <p className="text-kv-text leading-relaxed">{selectedWorkshop.desc}</p>
                      </div>
                    </div>
                  )}
                </div>
