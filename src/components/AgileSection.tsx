...
const steps: Step[] = [
  ...
  },
  {
    ...
    desc: 'Команда переводит идею в конкретный план: составляется перечень задач, распределяются роли участников, устанавливаются контрольные точки. Основной документ этапа — паспорт проекта, который фиксируется в системе КвантЛаб.',
    tasks: [
      'Составление и расстановка приоритетов задач',
      'Распределение ролей и ответственности в команде',
      'Заполнение паспорта проекта в КвантЛаб',
  ...
  },
  {
    ...
    desc: 'Готовый прототип направляется куратору через платформу КвантЛаб. Куратор изучает материалы, формулирует замечания и предложения по доработке. На основе полученной обратной связи команда корректирует план задач.',
    tasks: [
      'Загрузка материалов в систему КвантЛаб',
